...
data class AiConfig(
    ...
    /**
     * مسار ملف الموديل المحلي (`.task`) على الجهاز.
     *
    ...
    /**
     * معالج الموديل المحلي: `DEFAULT` أو `CPU` أو `GPU`.
     *
     * الفرق حقيقي مش تجميلي: كارت الشاشة في التليفونات الحديثة بيطلّع
     * الرمز أسرع بمرّات من المعالج. بس ذاكرته أقل، فموديل كبير عليه
     * بيفشل التحميل خالص بدل ما يبقى بطيء — عشان كده الاختيار مكشوف
     * للمستخدم بدل ما نقرّر له.
     */
    val localBackend: String = "DEFAULT"
) {
    /**
     * جاهز للاستخدام؟
     *
