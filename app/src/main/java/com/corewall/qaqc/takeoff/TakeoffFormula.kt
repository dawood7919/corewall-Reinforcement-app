...
/**
 * الأسماء في الحصر لاتينية بس.
 *
 * السبب مش تفضيل لغة: الصيغة بتستدعي البند **باسمه** كتوكن جوّه نص
 * زي `slab_area * bar_count`، والاسم العربي بيخلّي التوكن يتكتب من
 * اليمين لليسار وسط تعبير بيتقرا من الشمال لليمين — فمكان النجمة
 * والقوس بيبقى ملخبط بصريًا وانت بتراجع صيغة غلط. اللاتيني بيشيل
 * الالتباس ده من أصله.
 */
fun isLatinTakeoffName(name: String): Boolean {
    val trimmed = name.trim()
    if (trimmed.isBlank()) return false
    if (trimmed.none { it in 'a'..'z' || it in 'A'..'Z' }) return false
...
/** رسالة الرفض — بتتعرض تحت الخانة زي ما هي. */
const val LATIN_NAME_ERROR = "الاسم لازم يكون إنجليزي: حروف وأرقام و _ - . بس"
