...
enum class TakeoffTool {
    ...
    /**
     * بُعد مرجعي — ثلاث نقط (طرف، طرف، إزاحة الخط). بيقيس المسافة الحقيقية
     * بس **مش بند حصر**: مايدخلش في الإجماليات ولا الـBOQ ولا مراجع
     * الصيغ. لو دخل، أي قياس مرجعي حطّه المستخدم للتوضيح كان هيتحسب
     * غلط كأنه كمية فعلية.
     */
    DIMENSION;
    ...
    /** بند حصر حقيقي بيتحسب في الإجماليات/الـBOQ/مراجع الصيغ. */
    val isQuantity: Boolean get() = this != DEDUCT && this != DIMENSION
}
