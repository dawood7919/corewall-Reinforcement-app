...
// ═══════════════════════════════════════════════ الرسم للتصدير (Android Canvas)

/**
 * نفس الأشكال بالظبط، بس على `android.graphics.Canvas` للتصدير.
 *
 * ليه مكرّرة: التصدير بيرسم على كانفاس أندرويد عادي مش على `DrawScope`.
 * المهم إن المنطق (الأبعاد والزوايا ونصف قطر السحابة) مشتق من نفس القواعد،
 * فاللي بتشوفه على الشاشة هو اللي بيطلع في الملف.
 */
fun drawAnnotationAndroid(
    canvas: android.graphics.Canvas,
    annotation: PdfAnnotationEntity,
    normalized: List<Float>,
    pageWidthPx: Float,
    pageHeightPx: Float,
    /** بكسل التصدير لكل نقطة PDF — عشان السُمك يطلع مظبوط. */
    pxPerPoint: Float
) {
    if (normalized.size < 4) return
    val pts = (normalized.indices step 2).mapNotNull { i ->
        if (i + 1 >= normalized.size) null
        else android.graphics.PointF(normalized[i] * pageWidthPx, normalized[i + 1] * pageHeightPx)
    }
    if (pts.size < 2) return

    val tool = PdfTool.fromId(annotation.tool)
    val stroke = (annotation.strokeWidth * pxPerPoint).coerceAtLeast(1f)
    ...
    when (tool) {
        ...
        PdfTool.RECT -> canvas.drawRect(rect, paint)
        PdfTool.ELLIPSE -> canvas.drawOval(rect, paint)
        PdfTool.LINE -> canvas.drawLine(first.x, first.y, last.x, last.y, paint)
        PdfTool.ARROW -> {
            ...
            listOf(angle + ARROW_SPREAD, angle - ARROW_SPREAD).forEach { a ->
                canvas.drawLine(
                    last.x, last.y,
                    last.x - head * cos(a), last.y - head * sin(a),
                    paint
                )
            }
        }
        ...
        PdfTool.CLOUD -> {
            val radius = (min(rect.width(), rect.height()) / 8f).coerceIn(stroke * 2.5f, 28f)
            val step = radius * 1.7f
            val path = android.graphics.Path()
            fun arcs(fx: Float, fy: Float, tx: Float, ty: Float) {
                val len = hypot(tx - fx, ty - fy)
                if (len <= 0f) return
                val count = (len / step).toInt().coerceAtLeast(1)
                val dx = (tx - fx) / count
                val dy = (ty - fy) / count
            ...
            arcs(rect.left, rect.top, rect.right, rect.top)
            arcs(rect.right, rect.top, rect.right, rect.bottom)
            arcs(rect.right, rect.bottom, rect.left, rect.bottom)
            arcs(rect.left, rect.bottom, rect.left, rect.top)
            canvas.drawPath(path, paint)
        }
    }
}
