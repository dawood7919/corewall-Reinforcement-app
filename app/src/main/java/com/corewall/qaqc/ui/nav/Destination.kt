...
sealed interface Dest {
    ...
    /**
     * مفتاح ثابت لحفظ حالة الشاشة.
     *
     * لازم يبقى **نفسه** لنفس الشاشة عبر التنقّل، ومختلف بين الشاشات.
     * اسم الصنف + أي مُعرّف (زي مسار الملف) بيحقّقوا الاتنين: تفتح ملفين
     * مختلفين تلاقي لكل واحد حالته، وترجع لنفس الملف تلاقي مكانك.
     */
    val stateKey: String get() = toString()
     ...
     * قايمة أقسام الحصر.
     *
     * `Root` عشان تقدر تبقى شاشة البداية في نسخة الحصر المنفصلة —
     * `Navigator` بيقبل جذر بس. مش في الشريط السفلي (`TabSpec`)، والدرج في
     * النسخة الكاملة بيفتحها بـ`push` مش كتبويب، فسلوكها هناك ما اتغيّرش.
     */
    data object Takeoff : Root {
